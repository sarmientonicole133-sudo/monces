...
Route::prefix('admin')->middleware(['auth', 'admin', 'otp'])->name('admin.')->group(function () {
    Route::get('/dashboard', function () {
...
// Cart routes
Route::prefix('cart')->name('cart.')->middleware(['auth', 'otp'])->group(function () {
...
// Checkout routes
Route::prefix('checkout')->name('checkout.')->middleware(['auth', 'otp'])->group(function () {
    Route::get('/', [CheckoutController::class, 'index'])->name('index');
    Route::post('/', [CheckoutController::class, 'store'])->name('store');
});

// Payment routes
Route::prefix('payment')->name('payment.')->middleware(['auth', 'otp'])->group(function () {
...
Route::get('/test-order', function () { 
    $order = \App\Models\Order::with('items.product')->first(); 
    return response()->json([ 
        'order_id' => $order->id, 
...
Route::get('/test-email', function () {
    ...
    try {
        \Illuminate\Support\Facades\Mail::to('davepalola1@gmail.com')->send(new \App\Mail\TestEmail('This is a test message from your Aldrin app!'));
        return '<h3>Email sent successfully!</h3>';
    } catch (\Exception $e) {
        // Log the full error for debugging
        \Illuminate\Support\Facades\Log::error('Email sending failed: ' . $e->getMessage());
        return '<h3>Error sending email:</h3><p>' . $e->getMessage() . '</p><p>Check the logs for more details.</p>';
    }
})->name('test.email');